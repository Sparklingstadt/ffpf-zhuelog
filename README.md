...
- **制限**: 固定モデル `gpt-5-mini`、最大出力4,000トークン、サーバー45秒、送信本文8KiB。同一プロセス内で1キー同時1件・1分10件、全体同時16件に制限します。キーのSHA-256ダイジェストとカウンターだけを一時メモリーに保持し、DBには保存しません。サーバーレスの別インスタンス間では制限を共有しないため、厳密な課金上限ではありません。Vercelの無料枠やOpenAIの利用上限によって利用できない場合があります。
