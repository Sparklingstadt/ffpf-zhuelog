...
import { formatLogDateKey, getLogDateHref } from "@/presentation/presenters/log-date-presenter";
      ...
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-6 px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
        <header className="flex flex-col gap-5 border-b pb-6 sm:flex-row sm:items-end sm:justify-between">
          <div className="space-y-3">
            ...
            <div>
              <h1 className="text-3xl font-semibold tracking-tight">日付別の学習ノート</h1>
              <p className="mt-2 text-sm text-muted-foreground">学習文を登録した日から一覧を開けます。</p>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <Button asChild variant="outline" size="sm">
              <Link href="/"><ArrowLeft /> 学習ノートへ</Link>
            </Button>
        ...
        {dates.length === 0 ? (
          ...
          <div className="grid gap-3">
            {dates.map((group) => (
              <Link key={formatLogDateKey(group.date)} href={getLogDateHref(group.date)} className="group">
                <Card className="transition-colors group-hover:bg-muted/40">
                  <CardContent className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium">{formatLogDateKey(group.date)}</p>
                        <p className="text-sm text-muted-foreground">{group.count}件の学習ノート</p>
                      </div>
                    </div>
